package com.xyd.project.android.fragment;
...
import xydproject.pluginutil.util.FileUtils;
...
public class ApkFragment extends ListFragment implements ServiceConnection {
    ...
    private void startLoadInner() {
        ...
        new Thread("ApkScanner") {
            @Override
            public void run() {
                List<File> apkList = new ArrayList<>(10);
                try {
                    String[] files2 = getActivity().getAssets().list("");
                    for (String apk : files2) {
                        if (apk.endsWith(".apk")) {
                            FileUtils.copyAssetsToSdcard(getActivity(), apk);
                        }
                    }
                } catch (IOException e) {
                    e.printStackTrace();
